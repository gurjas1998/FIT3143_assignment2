...
struct valuestruct { 
	...
	int time_seconds; // Time in seconds to pass to master
    double time_nanoseconds; // ^ but nanoseconds
};
...
int master_io(MPI_Comm world_comm, MPI_Comm comm)
{
	...
	totalMessages = (int*)malloc(size * sizeof(int)); // Number of times a particular process has sent a request to master
    ...
    while ((time(NULL) - startTime) < secs){
        ...
        // time_taken represents difference between master's current time and time from node
		...
		// The following if statements are to determine valid adjacent nodes 
		if((fire_rank%n)-1 >=0){
		...
		fprintf(pInfile, "\nTotal Messages sent between reporting node and base station : %d\n",totalMessages[fire_rank]);
		...
		fprintf(pInfile, "--------------------------------\n");
		skip = false;
		total_comm_time = total_comm_time + time_taken;
		no_msgs = no_msgs + 2; // + 2 because requesting node and master always share 2 messages
	}
	fprintf(pInfile, "--------------------------------\n");
	fprintf(pInfile, "--------------------------------\n");
	fprintf(pInfile, "\nSUMMARY OF LOGS:\n");
	fprintf(pInfile, "\nNumber of messages passed throughout network from alert: %d\n", no_msgs);
	fprintf(pInfile, "Number of total alerts (true and false): %d\n", iteration);
	fprintf(pInfile, "Total communication time (between adjacent nodes and from requesting node to base station) from alert: %f seconds\n", total_comm_time);
	fprintf(pInfile, "\nEND OF REPORT\n");
...
int slave_io(MPI_Comm world_comm, MPI_Comm comm)
{
	...
	while(!flag) {
        ...
        struct timespec start, adj_nodes_start, adj_nodes_end;
        clock_gettime(CLOCK_MONOTONIC, &adj_nodes_start);
	    ...
	    clock_gettime(CLOCK_MONOTONIC, &adj_nodes_end);
        
        // Take the difference in seconds and nseconds for adjacent node communication
        double time_taken_adj_sec = adj_nodes_end.tv_sec - adj_nodes_start.tv_sec;
		double time_taken_adj_nsec = adj_nodes_end.tv_nsec - adj_nodes_end.tv_nsec;
	    ...
	    if(temp >= 80){
	        ...
	        if(inRange >=2){
	            ...
	            /* Here we 'add on' the time taken to communicate with adjacent nodes
	               By subtracting it, we get a bigger difference when we subtract it 
	               from time obtained during master's recv call */
	            values.time_seconds = start.tv_sec - time_taken_adj_sec;
	            values.time_nanoseconds = start.tv_nsec - time_taken_adj_nsec;
