...
int master_io(MPI_Comm world_comm, MPI_Comm comm)
{
	...
	int blocklen[7] = {1,1,1,1,1,1,1}; // Len of list elements?
	MPI_Aint disp[7]; // To store address of parameters
	...
	MPI_Get_address(&values.start, &disp[6]);
	...
	// Create MPI struct
	MPI_Type_create_struct(7, blocklen, disp, type, &Valuetype);
